...
function buildReport(data, analysis, reportUrl) {
  const serverHeaders = data.server.headers;
  const low = data.clientHints.lowEntropy || {};
  const high = data.clientHints.highEntropy || {};
  const lines = [
    ...
    reportLine("报告网址（7 天内有效）", reportUrl),
...
async function createReportLink(data) {
  ...
  return new URL(payload.path, location.origin).href;
}
...
async function collect() {
  ...
  try {
    ...
    const data = { ...local, clientHints, server };
    if (version !== collectionVersion) return;
    currentData = data;
    render(data);
    const reportUrl = await createReportLink(data);
    if (version !== collectionVersion) return;
    currentReport = buildReport(data, data.analysis, reportUrl);
    elements.copyButton.textContent = "复制完整报告";
    elements.copyButton.disabled = false;
  } catch {
    if (version !== collectionVersion) return;
    currentReport = "";
    elements.collectionStatus.textContent = currentData
      ? "诊断信息已采集，报告网址生成失败，请刷新重试"
      : "报告生成失败，请刷新重试";
