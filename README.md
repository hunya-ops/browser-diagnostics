...
`/api/reports` 只接受同源请求、限定结构的诊断数据和最多 64 KiB 的内容。报告保存在 `REPORTS` KV 中，设置 7 天 TTL；`/r/:id` 将数据渲染为带快速结论和分类明细的只读页面，并禁止搜索引擎索引。任何获得分享链接的人都可以在有效期内查看对应报告。
